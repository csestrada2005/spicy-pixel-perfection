    ...
    <div className={`relative w-full rounded-2xl bg-amarillo-suave text-negro shadow-[6px_6px_0px_#CA8A04] ${compact ? "max-w-[224px] p-3 pb-5" : "max-w-[340px] p-5 pb-8"}`}>
      <div className={`relative flex items-center justify-center overflow-visible ${compact ? "h-32" : "h-44"}`}>
        <img
          src={product.image}
          alt={product.flavor}
          className={`w-auto object-contain drop-shadow-md ${product.id === "fresh-lemon" ? "absolute left-1/2 top-[42%] h-[420px] -translate-x-1/2 -translate-y-1/2" : product.id === "mixed-berries" ? "absolute left-1/2 top-[36%] h-[420px] -translate-x-1/2 -translate-y-1/2" : product.id === "cherry-lemon" ? "absolute left-1/2 top-[22%] h-[342px] -translate-x-1/2 -translate-y-1/2" : compact ? "h-44" : "h-44"}`}
          loading="lazy"
        />
      </div>
